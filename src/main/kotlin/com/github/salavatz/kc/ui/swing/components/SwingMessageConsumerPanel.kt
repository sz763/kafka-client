...
import com.github.salavatz.kc.core.impl.ContainsDataFilter
import com.github.salavatz.kc.core.impl.NoopDataFilter
import com.github.salavatz.kc.ui.MessageConsumerPanel
import com.github.salavatz.kc.ui.PreviewService
import com.github.salavatz.kc.ui.swing.utilities.DateTimeConverterService
import com.github.salavatz.kc.ui.swing.utilities.KeyPressed
import com.github.salavatz.kc.ui.swing.utilities.MousePressed
import com.github.salavatz.kc.ui.swing.utilities.NotEditableTableModel
...
@Component
class SwingMessageConsumerPanel @Autowired constructor(
...
) : MessageConsumerPanel, SwingPanel("Events") {

    private val timestampHeader = "Timestamp"
    private val keyHeader = "Key"
    ...
    private fun initTable() {
        tableModel.addColumn(timestampHeader)
        tableModel.addColumn(keyHeader)
        tableModel.addColumn(valueHeader)
        setColumnPreferredSize(timestampHeader, 150, 100, 200)
        setColumnPreferredSize(keyHeader, 300, 100, 300)
        ...
        table.addMouseListener(MousePressed { e ->
            if (e.clickCount == 2) {
                preview.show(
                    table.getValueAt(table.selectedRow, table.getColumn(keyHeader).modelIndex) as String,
    ...
    private fun handleKeyFilter() {
        filterPopup("$keyHeader should contain", keyFilterValue)?.let {
            keyFilterValue = it
            keyFilter = ContainsDataFilter(it)
            updateHeader(it, keyHeader)
        }
    }

    private fun handleValueFilter() {
        filterPopup("$valueHeader should contain", valueFilterValue)?.let {
            valueFilterValue = it
            valueFilter = ContainsDataFilter(it)
            updateHeader(it, valueHeader)
        }
    }
