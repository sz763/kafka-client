...
@Component
class SerdeProvider @Autowired constructor(private val config: Config) : InitializingBean {
    //TODO make it lazy
    private lateinit var serdes: Map<String, Serde<*>>
    private lateinit var transformers: Map<String, KafkaTimeBasedTransformer>
    fun keySerde(): Serde<*> = serdeOf(config.keySerde())

    fun valueSerde(): Serde<*> = serdeOf(config.valueSerde())
    ...
    private fun serdeOf(serdeName: String): Serde<*> {
